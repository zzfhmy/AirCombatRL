...
from .distributions import BetaShootBernoulli, Categorical, DiagGaussian, Bernoulli, MultivariateGaussian, MultivariateGaussianMixtureModel
from .distributions import FixedNormal, FixedMultivariateNormal
...
class ACTLayer(nn.Module):
    ...
    def evaluate_actions_var(self, x, action, active_masks=None, **kwargs):
        if self._mlp_actlayer:
            x = self.mlp(x)
        policyMean, policyVar, _ = self.action_out.get_mean_var_logvar(x)
        policyNoGrad = policyMean.detach()
        action_dist = FixedNormal(policyNoGrad, policyVar)
        action_log_probs = action_dist.log_probs(action)
        return action_log_probs

    def evaluate_actions_mean(self, x, action, active_masks=None, **kwargs):
        if self._mlp_actlayer:
            x = self.mlp(x)
        policyMean, policyVar, _ = self.action_out.get_mean_var_logvar(x)
        policyVarNoGrad = policyVar.detach()
        action_dist = FixedNormal(policyMean, policyVarNoGrad)
        action_log_probs = action_dist.log_probs(action)
        return action_log_probs

    def get_meanvar(self, x, deterministic=False, **kwargs):
        policyMean, policyVar, policyLogVar = self.action_out.get_mean_var_logvar(x)
        return policyMean, policyVar, policyLogVar
...
# 多元高斯的行动层
class MultivarACTLayer(ACTLayer):
    def __init__(self, act_space, input_dim, hidden_size, activation_id, gain):
        super().__init__(act_space, input_dim, hidden_size, activation_id, gain)
        print("using Multivar ACTLayer")
        if isinstance(act_space, gym.spaces.Box):
            action_dim = act_space.shape[0]
            self.action_out = MultivariateGaussian(input_dim, action_dim, gain)
    ...
    def evaluate_actions_var(self, x, action, active_masks=None, **kwargs):
        ...
        policyMean, policyVar,_ = self.action_out.get_mean_var_logvar(x)
        policyNoGrad = policyMean.detach()
        action_dist = FixedMultivariateNormal(policyNoGrad, policyVar)
        action_log_probs = action_dist.log_probs(action)
        return action_log_probs
    ...
    def evaluate_actions_mean(self, x, action, active_masks=None, **kwargs):
        ...
        policyMean, policyVar,_ = self.action_out.get_mean_var_logvar(x)
        policyVarNoGrad = policyVar.detach()
        action_dist = FixedMultivariateNormal(policyMean, policyVarNoGrad)
        action_log_probs = action_dist.log_probs(action)
        return action_log_probs
...
# 混合高斯模型的行动层
class MultivarGmmACTLayer(ACTLayer):
    def __init__(self, act_space, input_dim, hidden_size, activation_id, gain, num_components):
        super().__init__(act_space, input_dim, hidden_size, activation_id, gain)
        print("using MultivarGmm ACTLayer")
        if isinstance(act_space, gym.spaces.Box):
            action_dim = act_space.shape[0]
            self.action_out = MultivariateGaussianMixtureModel(input_dim, action_dim, gain, num_components)
        
    def evaluate_actions_inkl(self, x, action, active_masks=None, **kwargs):
        """
        多加一个内部kl散度的估计
        """
        if self._mlp_actlayer:
        ...
        if active_masks is not None:
            ...
            dist_entropy = action_dist.entropy() / action_log_probs.size(0)
        # 近似内部KL散度
        approx_inter_kl = action_dist.approx_inter_kl(action)

        return action_log_probs, dist_entropy, approx_inter_kl
