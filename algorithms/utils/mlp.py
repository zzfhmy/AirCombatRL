...
# SAC Feature extraction module
class MLPqBase(nn.Module):
    def __init__(self, obs_space, act_space, hidden_size, activation_id, use_feature_normalization):
        super(MLPqBase, self).__init__()
        self._hidden_size = hidden_size
        self._activation_id = activation_id
        self._use_feature_normalization = use_feature_normalization
        ...
        input_dim = self.obs_flattener.size
        # 计算输入维度时加入动作空间的维度
        input_dim = input_dim + act_space.shape[0]
        if self._use_feature_normalization:
            self.feature_norm = nn.LayerNorm(input_dim)
        self.mlp = MLPLayer(input_dim, self._hidden_size, self._activation_id)
