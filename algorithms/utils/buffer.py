...
from .vcse import PBE,VCSE,TorchRunningMeanStd
...
class ReplayBuffer(Buffer):
    ...
    def after_update(self):
        """Copy last timestep data to first index. Called after update to model.将最后一个时间步长数据复制到第一个索引。更新到模型后调用"""
    ...
    def compute_returns(self, next_value: np.ndarray):
        ...
        if self.use_proper_time_limits:
            ...
            if self.use_gae:
                ...
                gae = 0
                for step in reversed(range(self.rewards.shape[0])):
                    # logging.info("obs.size: {}.\n" .format(self.obs[step].size(0)))        
                    td_delta = self.rewards[step] + self.gamma * self.value_preds[step + 1] * self.masks[step + 1] - self.value_preds[step]
                    gae = td_delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
                    self.returns[step] = gae + self.value_preds[step]
            else:
    ...
    @staticmethod
    def recurrent_generator(args, buffer: Union[Buffer, List[Buffer]], num_mini_batch: int, data_chunk_length: int):
        """
        ...
        一个循环生成器，用于生成以小批量排列的分块 RNN 训练的训练数据。
        该生成器按序列对数据进行洗牌。
        ...
        Returns:
            (obs_batch, actions_batch, next_obs_batch, masks_batch, old_action_log_probs_batch, advantages_batch, \
        ...
        rewards = np.vstack([ReplayBuffer._cast(buf.rewards) for buf in buffer])
        ...
        next_rnn_states_actor = np.vstack([ReplayBuffer._cast(buf.rnn_states_actor[1:]) for buf in buffer])
        next_rnn_states_critic = np.vstack([ReplayBuffer._cast(buf.rnn_states_critic[1:]) for buf in buffer])
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                next_rnn_states_actor_batch.append(next_rnn_states_actor[ind])
                next_rnn_states_critic_batch.append(next_rnn_states_critic[ind])
            ...
            next_rnn_states_actor_batch = np.stack(next_rnn_states_actor_batch).reshape(N, *buffer[0].rnn_states_critic.shape[3:])
            next_rnn_states_critic_batch = np.stack(next_rnn_states_critic_batch).reshape(N, *buffer[0].rnn_states_critic.shape[3:])
            ...
            rewards_batch = ReplayBuffer._flatten(L, N, rewards_batch)
            value_preds_batch = ReplayBuffer._flatten(L, N, value_preds_batch)


            if args.algorithm_name == "ppo":    
                yield obs_batch, actions_batch, masks_batch, old_action_log_probs_batch, advantages_batch, \
                    returns_batch, value_preds_batch, rnn_states_actor_batch, rnn_states_critic_batch
            elif args.algorithm_name == "sac": 
                yield obs_batch, actions_batch, next_obs_batch, masks_batch, next_masks_batch, rewards_batch, \
                value_preds_batch, rnn_states_actor_batch, rnn_states_critic_batch, next_rnn_states_actor_batch, next_rnn_states_critic_batch
            elif args.algorithm_name == "ddpg": 
                yield obs_batch, actions_batch, next_obs_batch, masks_batch, next_masks_batch, rewards_batch, returns_batch, \
                value_preds_batch, rnn_states_actor_batch, rnn_states_critic_batch, next_rnn_states_actor_batch, next_rnn_states_critic_batch
            else:
