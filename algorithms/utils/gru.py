...
class GRULayer(nn.Module):
    ...
    # x: 输入张量，形状为 [T * N, input_size]，其中 T 是序列长度，N 是批处理大小。
    # hxs: 隐藏状态张量，形状为 [N, L, hidden_size]，其中 L 是 GRU 层的层数
    # masks: 掩码张量，形状为 [T * N, 1]，用于标记序列中的有效时间步。
    ...
    def forward(self, x: torch.Tensor, hxs: torch.Tensor, masks: torch.Tensor):
        ...
        # (T>1): x=[T * N, input_size], hxs=[N, L, hidden_size], masks=[T * N, 1]
            ...
            # 在三元状态下，Mannual reset hxs to zero 可能太慢而无法计算
