...
def _get_prepare_config(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--algorithm-name", type=str, default='sac', choices=["ppo", "sac", "ddpg","mappo"],
...
def _get_network_config(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--action-dim", type=str, default='4',
                       help="action")
...
def _get_ppo_config(parser: argparse.ArgumentParser):
    ...
    # 是否使用多维高斯
    group.add_argument("--use-multivar", action='store_true', default=False,
                       help='By default false. If set, use multivariate normal')
    # 是否使用高斯混合模型
    group.add_argument("--use-gmm", action='store_true', default=False,
                       help='By default false. If set, use multivariate gaussian mixture model.')
    # 高斯混合模型的成份数
    group.add_argument("--gmm-component", type=int, default=4,
                       help='number of components in the multivariate gaussian mixture model.')
    # 是否使用内部KL散度正则化
    group.add_argument("--use-inter-kl", action='store_true', default=False,
                       help='By default false. If set, use internel kl regularization.')
    # 内部KL散度正则化系数
    group.add_argument("--inter-kl-coef", type=float, default=0.1,
                       help='internel kl regularization term coefficient (default: 0.1)')
    # 是否使用内部KL余弦衰减
    group.add_argument("--use-cos-schedule", action='store_true', default=False,
                       help='By default false. If set, use internel kl regularization cosine schedule.')
    return parser
